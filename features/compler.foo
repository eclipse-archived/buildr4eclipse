...
describe 'pde compiler' do
  ...
  it 'should identify from compile source directories' do
    ...
    write 'testing/com/example/Test.java', 'package com.example; class Test {}' 
    define 'foo' do
      lambda { compile.from 'src' }.should change { compile.compiler }.to(:pdec)
      lambda { test.compile.from 'testing' }.should change { test.compile.compiler }.to(:pdec)
    end
  end

  it 'should report the language as :java' do
    define('foo').compile.using(:pdec).language.should eql(:java)
  end

  it 'should set the target directory to target/classes' do
    define 'foo' do
      lambda { compile.using(:pdec) }.should change { compile.target.to_s }.to(File.expand_path('target/classes'))
    end
  end
...
describe Project, '#javadoc' do
  ...
  it 'should respond to from() and add project\'s sources and dependencies' do
    ...
    define 'foo' do
      compile.using(:pdec)
      define('bar') { compile.using(:pdec).with 'group:id:jar:1.0' }
  ...
  it 'should include compile dependencies' do
    define('foo') { compile.using(:pdec).with 'group:id:jar:1.0' }
    project('foo').javadoc.classpath.map(&:to_spec).should include('group:id:jar:1.0')
  end
